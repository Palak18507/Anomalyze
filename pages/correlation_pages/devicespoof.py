...
# --- Device Spoofing Detection Logic ---
def detect_device_spoofing(cdr_df):
    imei_counts = cdr_df.groupby('calling_number')['imei'].nunique().reset_index()
    spoofed_numbers = imei_counts[imei_counts['imei'] > 1]['calling_number']
    spoofed_cdrs = cdr_df[cdr_df['calling_number'].isin(spoofed_numbers)]
    return spoofed_cdrs

# --- Map Visualization ---
def plot_spoofing_on_map(result_df):
    if result_df.empty:
...
def generate_pdf_report(result_df):
    ...
    pdf.cell(0, 10, "Device Spoofing Detection Report", ln=1, align='C')
    pdf.set_font("Arial", "", 12)
    pdf.cell(0, 10, f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", ln=1)
    pdf.ln(5)
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Detected Multiple IMEI Usage (Possible Device Spoofing)", ln=1)
...
st.set_page_config(page_title="Device Spoofing Detection Forensic Tool", layout="wide")
st.title("Device Spoofing Detection Forensic Tool: Multi-File CDR Analysis")

st.markdown("""
This tool detects possible device spoofing by identifying multiple IMEIs associated with the same MSISDN (SIM) across CDR logs.
...
if run_analysis and uploaded_files and len(uploaded_files) >= 2:
    ...
    result_df = detect_device_spoofing(st.session_state['cdr_df'])
    st.session_state['result_df'] = result_df
    st.session_state['analysis_done'] = True
    completion_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    st.success(f"Device Spoofing Analysis completed at {completion_time}")
    st.write("---")
...
if st.session_state.get('result_df') is not None and st.session_state.get('analysis_done'):
    ...
    if not result_df.empty:
        st.error(f"Possible Device Spoofing Detected! {result_df['calling_number'].nunique()} numbers affected.")
        st.markdown("### Detected Device Spoofing Records")
...
|---------------|-------------|
| **Map** | Shows flagged device spoofing events on a map by city and coordinates. |
""")

        st.subheader("Device Spoofing Locations Map")
        plot_spoofing_on_map(result_df)
        ...
        if st.button("Generate PDF Report"):
            ...
            with open(pdf_path, "rb") as f:
                st.download_button(
                    ...
                    data=f.read(),
                    file_name="device_spoofing_report.pdf",
        ...
        st.success("No device spoofing detected.")
else:
